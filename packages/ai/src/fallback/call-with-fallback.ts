...
export interface FallbackModels {
  primary: string;
  // Undefined = fallback not configured for this call site, see
  // ai-scoring.md §8.5 — behaves identically to not having this module at
  // all (mvp-scope.md §9.1: unconfigured behavior must stay unchanged).
  fallback?: string;
}
...
// Embedding, see ai-scoring.md §8.4 for why. Retries the primary (free)
...
// (ai-scoring.md §8.5). Returns null when nothing worked — every existing
